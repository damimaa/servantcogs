...
class AutoRoomCommands(MixinMeta, ABC):
    ...
    @commands.group(name="voicemaster", aliases=["vc", "voice", "vm"])
    @commands.guild_only()
    ...
    @autoroom.command(name="name", aliases=["rename"])
    async def autoroom_name(self, ctx: commands.Context, *, name: str):
        ...
        embed = discord.Embed(description=f":white_check_mark: {ctx.author.mention}: Your voice channel has been renamed to `{name}`", color=0xa4eb78)
    ...
    async def autoroom_bitrate(self, ctx: commands.Context, kbps: int):
        ...
        if bps != autoroom_channel.bitrate:
            ...
            embed = discord.Embed(description=f":white_check_mark: {ctx.author.mention}: Your voice channel bitrate has been set to `{kbps}`", colour=0xa4eb78)
            await ctx.send(embed=embed)
    ...
    async def autoroom_users(self, ctx: commands.Context, user_limit: int):
        ...
        if limit != autoroom_channel.user_limit:
            ...
            embed = discord.Embed(description=f":white_check_mark: {ctx.author.mention}: Your voice channel limit has been updated to `{user_limit}`", color=0xa4eb78)
            await ctx.send(embed=embed)

    @autoroom.command(name="unlock")
    async def public(self, ctx: commands.Context):
        """Make your AutoRoom public."""
        await self._process_allow_deny(ctx, self.perms_public)
        await ctx.tick()

    @autoroom.command(name="lock")
    ...
    @autoroom.command(aliases=["permit"])
    async def allow(
        ...
        embed = discord.Embed(description=f":white_check_mark: {ctx.author.mention}: Successfully granted permissions to join your voice channel to **{member_or_role}**", color=0xa4eb78)
        await ctx.send(embed=embed)
        ...
        if await self._process_allow_deny(
            ...
            for member in channel.members:
                if not channel.permissions_for(member).connect:
                    ...
                    embed = discord.Embed(description=f":white_check_mark: {ctx.author.mention}: Successfully removed permissions to join your voice channel to **{member_or_role}**", color=0xa4eb78)
                    await ctx.send(embed=embed)
    ...
    def get_autoroom_type(autoroom: discord.VoiceChannel, role: discord.Role):
        """Get the type of access a role has in an AutoRoom (unlocked, locked, private, etc)."""
